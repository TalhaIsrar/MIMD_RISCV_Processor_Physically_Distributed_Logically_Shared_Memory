module soc (
  ...
  picorv32_ahb riscv_1 (
    ...
    .HADDR(HADDR_M0), .HBURST(HBURST), .HMASTLOCK(HMASTLOCK), .HPROT(HPROT), .HSIZE(HSIZE_M0), .HTRANS(HTRANS_M0), .HWDATA(HWDATA_M0), .HWRITE(HWRITE_M0),
    .HRDATA(HRDATA), .HREADY(HREADY_M0), .HRESP(HRESP)                                   
  ...
  dma_controller dma_controller_inst(
    ...
    .HSEL_S(HSEL_DMA), .HREADY_S(HREADY), .HADDR_S(HADDR), .HWDATA_S(HWDATA),
